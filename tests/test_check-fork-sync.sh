#!/usr/bin/env bash
# Tests for check-fork-sync script

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SCRIPT="$SCRIPT_DIR/check-fork-sync"

echo "Testing check-fork-sync..."
...
if [[ -n "$output" ]]; then
	...
	echo "⚠ INFO: No fork repos need syncing"
fi
...
echo ""
echo "All tests for check-fork-sync passed!"
